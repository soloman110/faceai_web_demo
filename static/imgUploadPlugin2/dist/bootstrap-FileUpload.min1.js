...
;(function(i, e, g, o) {
    ...
    s = {
        ...
        uploadStart: function(a) {
            i("#"+ a).find(".fileupload-add, .fileupload-start, .fileupload-cancel, .fileupload-remove").attr("disabled", "disabled"),
            v[a].options.hiddenInput && i.each(v[a].options.hiddenInput, function(e, o) {
                console.log("hiddenInput", e, o);
            ...
            (
                ...
                //formData 초기화
                v[a].formData = new FormData,
                "function" == typeof v[a].options.onUploadProgress && v[a].options.onUploadProgress.call(this,v[a].overallProgressBar),
                i.each(v[a].arrayFiles, function(e, o) {
                    v[a].formData.append(v[a].options.inputName + "[]", o);
                }),
                 r(a, "")
        ...
        removeFile: function(e, o) {
            v[e].arrayLength <= 1 ? s.resetUpload(e) : (i("#" + o).fadeOut("slow", "linear"),
            i("#" + o + " .alert").fadeOut("slow", "linear"),
            i("#" + o).remove(),
            delete v[e].arrayFiles[o],
            v[e].arrayLength = --v[e].arrayLength),
            "function" == typeof v[e].options.onFileRemoved && v[e].options.onFileRemoved.call(this)
        },
        resetUpload: function(e) {
            v[e].filePreviewTable.find("tbody").empty(),
            v[e].form[0].reset(),
            v[e].arrayFiles = {},
            v[e].arrayLength = 0,
            v[e].filePreviewTable.fadeOut("slow", "linear"),
            v[e].btnStart.fadeOut("slow", "linear"),
            v[e].btnCancel.fadeOut("slow", "linear"),
            i("#"+ e).find(".fileupload-previewrow .alert").fadeOut("slow", "linear"),
            i("#"+ e).find(".fileupload-add, .fileupload-start, .fileupload-cancel").removeAttr("disabled"),
            i("#"+ e).find(".fileupload-add").delay(800).fadeIn("slow", "linear"),
            v[e].overallProgressBar.find(".progress-bar-success").attr("aria-valuenow", 0).css("width", "0%"),
            v[e].overallProgressBar.fadeOut("slow", "linear"),
            v[e].btnReset.fadeOut("slow", "linear"),
            "function" == typeof v[e].options.onUploadReset && v[e].options.onUploadReset.call(this)
        }
    };
